...
from pdf_export_utils import create_export_ui
                ...
                if len(feature_cols) > 0:
                    ...
                    if st.button("🚀 分析を実行", type="primary", use_container_width=True):
                        with st.spinner("分析中..."):
                            ...
                            # Add PDF export button right after analysis
                            col1, col2, col3 = st.columns([1, 1, 2])
                            with col1:
                                if st.button("📄 PDFレポートを生成", type="secondary"):
                                    with st.spinner("PDFを生成中..."):
                                        try:
                                            # Prepare data for PDF
                                            pdf_generator = PDFReportGenerator()
                                            pdf_data = prepare_analysis_for_pdf(st.session_state.analysis_results)
                            ...
                            if 'pdf_report' in st.session_state and st.session_state.pdf_report:
                                with col2:
                                    st.download_button(
                                        ...
                                        file_name=f"mdv_analysis_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                    ...
                    if st.session_state.analysis_results is not None:
                        ...
                        with tabs[7]:
                            ...
                            if st.session_state.analysis_results is not None:
                                ...
                                if st.button("📝 解釈プロンプトを生成", type="primary"):
                                    ...
                                    # Get sample count from regression results
                                    sample_count = len(results.regression_results['X_train']) + len(results.regression_results['X_test'])
                                    
                                    analysis_summary = {
                                        "データ概要": {
                                            "サンプル数": sample_count,
                                            "説明変数": st.session_state.feature_names if st.session_state.feature_names else feature_cols,
                                            "ターゲット変数": target_col
                                        },
                                        "相関分析": {
                        ...
                        # Tab 9: PDF Export
                        with tabs[8]:
                            st.header("📄 PDFレポートエクスポート")
                            
                            st.info("分析結果をPDFレポートとして出力できます")
                            ...
                            with col1:
                                st.subheader("レポート設定")
                                include_correlation = st.checkbox("相関分析を含める", value=True)
                                include_regression = st.checkbox("回帰分析を含める", value=True)
                                include_tree = st.checkbox("決定木分析を含める", value=True)
                                include_pca = st.checkbox("主成分分析を含める", value=True)
                                include_diagnostics = st.checkbox("診断結果を含める", value=True)
                            
                            with col2:
                                st.subheader("出力オプション")
                                report_title = st.text_input(
                                    "レポートタイトル",
                                    value="MDV Share Analysis Report",
                                    help="PDFレポートのタイトルを設定します"
                                )
                                
                                author_name = st.text_input(
                                    "作成者名（任意）",
                                    value="",
                                    help="レポートの作成者名を入力します"
                                )
                            
                            # Generate PDF button
                            if st.button("📄 PDFレポートを生成", type="primary", use_container_width=True):
                                with st.spinner("PDFレポートを生成中..."):
                                    try:
                                        ...
                                        if report_title:
                                            pdf_data['title'] = report_title
                                        if author_name:
                                            pdf_data['author'] = author_name
                                        
                                        # Filter sections based on user selection
                                        if not include_correlation and 'correlation' in pdf_data:
                                            del pdf_data['correlation']
                                        if not include_regression and 'regression' in pdf_data:
                                            del pdf_data['regression']
                                        if not include_tree and 'tree' in pdf_data:
                                            del pdf_data['tree']
                                        if not include_pca and 'pca' in pdf_data:
                                            del pdf_data['pca']
                                        if not include_diagnostics and 'diagnostics' in pdf_data:
                                            del pdf_data['diagnostics']
                                        ...
                                        st.download_button(
                                            ...
                                            data=pdf_bytes,
                                            file_name=f"mdv_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                                            mime="application/pdf",
                                            use_container_width=True
                                        )
                                        
                                        # Show preview info
                                        st.success(f"レポートサイズ: {len(pdf_bytes) / 1024:.1f} KB")
                                        
                                    except Exception as e:
                                        st.error(f"PDF生成エラー: {str(e)}")
                                        st.exception(e)
                            
                            # Show last generated report if available
                            if 'pdf_report' in st.session_state and st.session_state.pdf_report:
                                st.markdown("---")
                                st.subheader("前回生成したレポート")
                                st.download_button(
                                    label="📥 前回のPDFレポートをダウンロード",
                                    data=st.session_state.pdf_report,
                                    file_name=f"mdv_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.pdf",
                                    mime="application/pdf",
                                    use_container_width=True
                        ...
                        # Tab 10: Chart and Table Export
                        with tabs[9]:
                            # Call the export UI function
                            create_export_ui(st.session_state.analysis_results, target_col)
